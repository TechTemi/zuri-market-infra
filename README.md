...
- [2. Pain Points and Solutions](#2-pain-points-and-solutions)
- [3. Repository Map](#3-repository-map)
- [4. DevSecOps Architecture](#4-devsecops-architecture)
- [5. Project Stages](#5-project-stages)
...
- [12. Capstone Deliverable Completion Verification](#12-capstone-deliverable-completion-verification)
- [13. Runbook](#13-runbook)
- [14. Production Readiness](#14-production-readiness)
- [15. Future Improvements](#15-future-improvements)
- [16. Final Verification Summary](#16-final-verification-summary)
...
These solutions directly address the original Zuri Market risks by replacing manual work with automation, moving secrets into managed stores, and proving every delivery step with evidence.
...
| 2 | GitHub Repo | GitHub Actions | Trigger pipeline | GitHub Actions | Workflow run | Branch trigger |
...
| 11 | k3s Cluster | Application Pods | Run containers | K8s | Frontend and backend pods | Self-healing workloads |
| 12 | K8s Service | Users | Expose application | Service or Ingress | Public access | Controlled ports |
...
|---|---|---|
| Local development | `.env` | Local-only developer configuration |
| Safe templates | `.env.example` | Shows required variables without real values |
| CI/CD | GitHub Actions Secrets | Stores credentials used by the pipeline |
| Runtime | AWS Secrets Manager and Kubernetes Secrets | Stores application runtime secrets |


| Secret | Stored In | Used By | Purpose | Security Decision |
|---|---|---|---|---|
| `DOCKERHUB_USERNAME` | GitHub Actions Secrets | CI/CD | DockerHub login | Do not store in workflow YAML |
| `DOCKERHUB_TOKEN` | GitHub Actions Secrets | CI/CD | Push Docker images | Use token instead of password |
| `AWS_ACCESS_KEY_ID` | GitHub Actions Secrets | CI/CD | AWS authentication | Store as encrypted secret |
| `AWS_SECRET_ACCESS_KEY` | GitHub Actions Secrets | CI/CD | AWS authentication | Store as encrypted secret |
| `KUBECONFIG` | GitHub Actions Secrets | CI/CD | k3s access | Do not commit kubeconfig |
| `SSH_PRIVATE_KEY` | GitHub Actions Secrets | CI/CD | EC2 access if needed | Restrict usage |
| `API_SECRET_KEY` | AWS Secrets Manager | Backend | Backend app secret | Runtime only |
| `STORE_NAME` | AWS Secrets Manager or ConfigMap | Application | Store configuration | Do not hardcode |
